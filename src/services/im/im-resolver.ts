...
const CALLOUT_TITLES: Record<string, string> = { warning: 'WARNING', caution: 'CAUTION', electric: 'ELECTRIC HAZARD', flammable: 'FLAMMABLE', info: 'INFO' };
 ...
 */
export const wrapBlockCallout = (blockType: string, contentHtml: string): string => {
  const icon = ISO_ICONS[blockType];
  if (!icon || !contentHtml) return contentHtml;
  const title = CALLOUT_TITLES[blockType] ?? blockType.toUpperCase();
