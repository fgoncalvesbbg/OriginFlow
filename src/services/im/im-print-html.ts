...
// Attribute-safe grade (escapes quotes too) — used here for both attribute values and plain
// text content, matching this file's pre-existing (over-cautious for text nodes) behaviour.
...
// ---------------------------------------------------------------------------
// ISO 7010 callout icons — shared source in ./iso-icons (also used by im-resolver.ts and
// InlineBlockEditor.tsx; src/modules/im-viewer/html.ts keeps its own byte-identical copy since
// that module must stay import-free of app code).
// ---------------------------------------------------------------------------

type CalloutVariant = 'warning' | 'danger' | 'caution' | 'electric' | 'flammable' | 'hot_surface' | 'info';
const ISO_ICONS: Record<CalloutVariant, string> = ISO_CALLOUT_ICONS;
