...
export const Breadcrumbs: React.FC = () => {
  ...
  return (
    <nav className="flex items-center text-xs text-slate-500 mb-6 animate-in fade-in slide-in-from-left-2 duration-300">
      <Link 
        to="/" 
        className="hover:text-blue-600 flex items-center gap-1 transition-colors hover:bg-slate-100 px-2 py-1 rounded"
      >
        <Home size={14} />
      ...
      {pathnames.map((value, index) => {
        ...
        return (
          <React.Fragment key={to}>
            ...
            {isLast ? (
              <span className="font-semibold text-slate-700 capitalize truncate max-w-[200px] px-1">
                {label}
              </span>
            ) : canLink ? (
              <Link 
                to={to} 
                className="hover:text-blue-600 capitalize transition-colors hover:bg-slate-100 px-2 py-1 rounded"
              >
                {label}
              </Link>
            ) : (
              <span className="capitalize text-slate-400 px-1">
                {label}
              </span>
            )}
          </React.Fragment>
